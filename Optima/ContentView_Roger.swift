...
var ActivityCategory: [String: [Activity]] = [
    "academic": [
        Activity(title: "study for 30 min", description: "happy", duration: 30, points: 10),
        Activity(title: "study for 40 min", description: "happy", duration: 40, points: 15)
    ],
    "fitness": [
        Activity(title: "pushup for 30 min", description: "happy", duration: 30, points: 10),
        Activity(title: "pushup for 40 min", description: "happy", duration: 40, points: 15)
    ],
    "household": [
        Activity(title: "clean for 30 min", description: "happy", duration: 30, points: 10),
        Activity(title: "clean for 40 min", description: "happy", duration: 40, points: 15)
    ],
    "wellness": [
        Activity(title: "read for 30 min", description: "happy", duration: 30, points: 10),
        Activity(title: "read for 40 min", description: "happy", duration: 40, points: 15)
    ]
]
...
struct NewTaskView: View {
    ...
    @State private var selectedActivityTitle: String = ""
                ...
                Section(header: Text("Task Details")) {
                    ...
                    .onChange(of: selectedCategory) { newCategory in
                        if newCategory == .custom {
                            ...
                            let key = newCategory.rawValue.lowercased()
                            if let activities = ActivityCategory[key], let firstActivity = activities.first {
                                selectedActivityTitle = firstActivity.title
                                title = firstActivity.title
                                description = firstActivity.description
                                duration = Double(firstActivity.duration)
                    ...
                    if selectedCategory == .custom {
                        ...
                        if let activities = ActivityCategory[selectedCategory.rawValue.lowercased()], !activities.isEmpty {
                            Picker("Title", selection: $selectedActivityTitle) {
                                ForEach(activities, id: \.id) { activity in
                                    Text(activity.title).tag(activity.title)
                                }
                            }
                            .onChange(of: selectedActivityTitle) { newTitle in
                                if let activity = activities.first(where: { $0.title == newTitle }) {
                                    title = activity.title
                                    description = activity.description
                                    duration = Double(activity.duration)
